...
namespace day06
{
    internal class Program
    {
        ...
        static void Main4()//stringbuilder
        {
            //字符串池：提高内存利用率
            //字符串的不可变性
        ...
        //字符反转
        private static string ReverseCha(string str)
        {
            char[] rev=str.ToArray();
            Array.Reverse(rev);
        ...
        {
            StringBuilder newBuilder = new StringBuilder(128);
            newBuilder.Append(str);
            bool[] asc=new bool[128];
            for(int i = 0; i < newBuilder.Length; i++)
            {
                if(asc[newBuilder[i]]==false)
                {
                    asc[newBuilder[i]] = true;
                }
                else
                {
                    newBuilder.Remove(i, 1);
                }
            }
        ...
        static void Main()
        {
            ...
            string result=RemoveRepeat(str);
